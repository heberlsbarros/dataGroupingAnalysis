transposicao <- function (alvo_analise) {
  ############################ SEÇÃO 1: INSTALAÇÃO E CARREGAMENTO DOS PACOTES NECESSÁRIOS ####################
  # Instalando os pacotes necessários:
  ## Listando os pacotes necessários
  pacotes <- c("openxlsx",#permite ler planilhas, formato .xlsx 
               "writexl",#permite criar planilhas, formato .xlsx
               ...
               "lubridate") #Traz ferramentas para manejo de strings, dataframes e, principalmente, datas.
  ### Checando se os pacotes estão instalados. Aqueles que não estão, serão automaticamente instalados.
  pacotes_instalados <- pacotes %in% rownames(installed.packages())#Cria um objeto que recebe o nome de todos pacotes instalados
  if (any(pacotes_instalados == FALSE)) {#Condicional que verifica se todos itens do objeto "pacotes_instalados" estão instalados (ou seja, TRUE)
    install.packages(pacotes[!pacotes_instalados])#Verifica se há pacotes não instalados comparando a lista de pacotes inicial com a lista de pacotes desejados instalados. Aqueles que figurarem na primeira lista, mas não na segunda, serão instalados.
  }
  ...
  ############################ SEÇÃO 2: IMPORTANDO  LAYOUTS E RESULTADOS E FAZENDO TRATAMENTO PRÉVIO DAS PLANILHAS ####################
  ...
  ## Recebendo todas informações de layout, dados brutos e data de plaqueamento de uma planilha ##
  ...
  prev_date_search <- gsub(".{5}$", "", nomes_arq) #Objeto prévio, necessário para extrair as datas de experimentos.
  date_search <-gsub(".*_", "", prev_date_search)#Cria um objeto com as datas dos experimentos.
  df_cheio <- cbind(nomes_arq, nomes_aba, date_search) # Agora é necessário juntar as duas colunas. A primeira é o nome dos arquivos, e a segunda é o nome das abas. Não aplicar nenhuma classificação para que a correspondência não fique errada! A função "cbind", seguida pelos objetos com o conteúdo das colunas, foi usado com sucesso.
  df_cheio <- as.data.frame(df_cheio) # Transformando df_final em dataframe.
  colnames(df_cheio) <- c("nomes_arquivos_xlsx", "respectivas_abas", "data_plaqueamento") # Alterando os nomes das colunas.
  ### Rodar separadamente
  rm (pesquisa_arq, nomes_aba, nomes_arq) # Removendo os objetos temporários.
  ### Rodar separadamente
  rm (arquivos_df, resultado_df, prev_date_search, date_search) # Esses objetos também não são mais necessários.
  ...
  df_final1 <- cbind(range_termos_df, df_final$nomes_arquivos_xlsx, df_final$respectivas_abas, df_final$data_plaqueamento) # Acrescentando uma coluna ao data.frame "df_final", com o número de linhas dele.
  ...
  colnames(df_final) <- c ("range", "arquivos", "abas", "data_plaqueamento") # Transformando os nomes das colunas do df_final.
  ### Removendo arquivos temporários
  rm (df_final2) # Removendo o df_final2.
  
  ############################ SEÇÃO 3: LOOP PARA CRIAÇÃO DOS ARQUIVOS TRANSPOSTOS ####################
  ...
  for (i in data2){ # Esse ciclo de for gerará uma lista que registrará os dados dos layouts e dados brutos diretamente das planilhas. Os dados serão extraídos e alocados em somente uma lista. Posteriormente, será preciso separar esses dados.
    ...
    raw_data <- readxl::read_excel(paste(path_planilha_original, i$arquivos,sep=""), sheet = i$abas, range = "B2:Y17", col_names = F)
    raw_data_combined <- bind_rows(raw_data)
    raw_data_long <- mutate (raw_data_combined,(row_num = row_number()))# A combinação dos símbolos "%>%" serve como o pipe do bash. Nesse caso, indica que o objeto "raw_data_long" será o resultado da manipulação do "raw_data_combined". A manipulação está descrita após os símbolos "%>%".
    raw_data_long <- pivot_longer(raw_data_long, cols = starts_with("..."), names_to = "col", values_to = "fluorescencia") # Linha que tira a database do formato "wide" (largo) para o formato "long" (comprido).
    final_data <- cbind(layout_long, raw_data_long, i$data_plaqueamento) #Aqui é a formação do data.frame contendo informações de layout e dados brutos.
    final_data <- final_data[complete.cases(final_data), ] #Essa linha de comando é responsável por tirar as linhas com "N/A" nelas, dispensando a necessidade de fazer isso pelo excel.
    output_planilha <- dplyr::select(final_data, c(tipo_celula, droga1, concentracao_droga1, droga2, concentracao_droga2, droga3, concentracao_droga3, replicata, fluorescencia, data_plaqueamento))
    id_file <- character() #Esse objeto receberá o nome contido no i$arquivos, retirando os últimos 5 dígitos, que correspondem a '.xlsx'". Esse objeto comporá o nome do arquivo transposto resultante.
    id_file <- gsub(".{5}$", "", i$arquivos)
    acessorio <- paste("./abas_transpostas/",id_file,"_",i$abas,".xlsx",sep="")
  ...
  } #Ao final deste loop, o resultado deverão ser arquivos salvos na pasta /abas_transpostas/. Considerando que esta Encerrar a função aqui.
