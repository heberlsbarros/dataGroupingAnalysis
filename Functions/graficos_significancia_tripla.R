graficos_significancia_tripla <- function (nome_da_planilha, tituloImagem, resposta) { #Definição dos elementos que serão chamados para execução da função. nome_da_planilha deverá conter o dataframe a ser analisado, "tituloImagem" deverá conter a identificação do grupo analisado (recomendo colocar o mesmo nome do dataframe para facilitar uma averiguação posterior de resultados; "resposta" é o tipo de leitura que foi realizada, sendo normalmente "fluorescencia")
  ######################==========INSTALAÇÃO E CARREGAMENTO DE PACOTES===========######################
  #Instalação e carregamento de pacotes necessários. Se for necessário acrescentar ou retirar pacotes à função, isso deverá ser feito no objeto abaixo, chamado pacotes. Não alterar o restante dessa seção.
  ...
  pacotes_instalados <- pacotes %in% rownames(installed.packages()) #Checa se os pacotes listados já estão instalados, resultando em um vetor lógico. Aqueles não instalados (FALSE) serão instalados automaticamente.
  if (any(pacotes_instalados == FALSE)) { #condicional que checa se há algum pacote não instalado e o instala
    install.packages(pacotes[!pacotes_instalados]) #indica que, dentre os pacotes listados (objeto pacotes), todos aqueles que não estiverem listados também no objeto "pacotes_instalados"
  }
  ...
  nome_da_planilha <- rbind (grupo_ct_dmso, grupo_ct_droga1, grupo_ct_droga2, grupo_ct_droga3, grupo_ct_droga12, grupo_ct_droga13, grupo_ct_droga23, grupo_comb)
  ...
  porcentagens_media <- as.numeric(c(coleta_media)) #Tirando o resultado do loop do formato de lista e colocando no formato númerico
  ...
  ######################==========ANOVA INICIAL (INDEPENDENTE DE TESTE DE LEVENE E SHAPIRO WIKL)==========######################
  anova_inicial <- aov(as.formula(paste(resposta, "~", "nome_comb")), data = nome_da_planilha) #Fórmula do ANOVA
  tukey_inicial <- TukeyHSD(anova_inicial)
  cat ("\n### A seguir uma análise de ANOVA que **INDEPENDE** de resultados de Levene e Normalidade (Shapiro-Wilk), seguido pos um pós-teste de Tukey comparando todos grupos entre si.###\n")
  print (anova_inicial)
  print (tukey_inicial) 
  ...
  nome_da_planilha$nome_comb <- factor(nome_da_planilha$nome_comb, levels = c ("CT DMSO", nome_ind1, nome_ind2, nome_ind3, nome_dup12,nome_dup13, nome_dup23,nome_comb), ordered = T)
    # Boxplot
  cat("\n### Boxplot ###\n")
  ...
  if (grepl ("Tukey", teste_e_posteste, fixed = T) == T) { #Ferramenta de busca
  ...
  ############===================FAZENDO O GRÁFICO DE BARRAS ABSOLUTO=====================#############
 ...
 ############ORDENAMENTO DO GRUPO DE PORCENTAGENS#############
 porcentagens_individual$nome_comb <- factor(porcentagens_individual$nome_comb, levels = c ("CT DMSO", nome_ind1, nome_ind2, nome_ind3, nome_dup12,nome_dup13, nome_dup23,nome_comb), ordered = T)
 
 ############CONFECÇÃO GRÁFICO RELATIVO#################
 ...
 ##Objeto que receberá o dado de linhagem celular
 vetor_linhagem <- as.character(nome_da_planilha$tipo_celula [1:8])
 
 ##Objeto que receberá o dado de identificação de cada grupo que compõe o dataframe analisado
 vetor_grupo <- as.character(resumo_grupos$nome_comb[1:8]) #Extrair o conteúdo dos grupos como caractere para um novo objeto
 
 ##Objeto que receberá o dado de n amostral por grupo
 vetor_n <- resumo_grupos$n[1:8]
 
 ##Objeto que receberá os valores de média de cada grupo
 vetor_resposta <- resumo_grupos$media [1:8]
 
 ##Objeto que receberá os valores de porcentagem média de cada grupo
 vetor_porcentagem <- resumo_porcentagens$media
 
 ##Confecção da lista de dados estatísticos que agrupam dados relevantes dos grupos analisados
...
##Exportando "lista_dados_estatisticos" como um arquivo excel (.xlsx)
 ...
 saveWorkbook(planilha_existente, file = paste(getwd(),"/relatorios_todas_combinacoes/todas_combinacoes_compilado_estatistico_", vetor_linhagem[1], "_", tituloImagem, ".xlsx", sep=""), overwrite = T) #Salva o arquivo planilha
 
 #Salvamento automático dos gráficos. Habilitar as seções abaixo somente se necessário para diminuir o espaço em disco utilizado.
 # ggsave(paste(getwd(),"/graficos_todas_combinacoes/todas_combinacoes_boxplot_",tituloImagem,".jpg", sep=""), plot = grafico_boxplot, width = 10, height = 7, dpi = 500) #Exporta a imagem de gráfico de boxplot
 # ggsave(paste(getwd(),"/graficos_todas_combinacoes/todas_combinacoes_qqplot_",tituloImagem,".jpg", sep=""), plot = qqplot_facet, width = 10, height = 7, dpi = 500) #Exporta a imagem de gráfico de qqplot
 # ggsave(paste(getwd(),"/graficos_todas_combinacoes/todas_combinacoes_colunasAbsoluto_",tituloImagem,".jpg", sep=""), plot = grafico_grupos, width = 10, height = 7, dpi = 500)  #Exporta a imagem de gráfico absoluto
 ggsave(paste(getwd(),"/graficos_todas_combinacoes/", vetor_linhagem[1], "_", "todas_combinacoes_colunasRelativo_",tituloImagem,".jpg",sep=""), plot = grafico_porcentagem, width = 10, height = 7, dpi = 500) #Exporta a imagem de gráfico relativo
 # write.csv(nome_da_planilha, paste("./dataframes/todas_combinacoes_dataframe",tituloImagem, ".csv", sep = ""), row.names = F, quote = F) #Exporta os dados estatísticos simplificados de cada grupo analisado em um formato mais leve para possível checagem posterior de resultado
 save(grafico_grupos, file = paste(getwd(),"/objetos_graficos/todas_combinacoes_fluorescencia_",vetor_linhagem[1], "_", "todas_combinacoes_colunasRelativo_",tituloImagem, ".RData", sep = "")) #Exporta o objeto que gerou o gráfico absoluto em formato .Rdata, usado posteriormente para montagem de outros tipos de gráficos
 save(grafico_porcentagem, file = paste(getwd(),"/objetos_graficos/todas_combinacoes_porcentagens_",vetor_linhagem[1], "_", "todas_combinacoes_colunasRelativo_",tituloImagem, ".RData", sep = "")) #Exporta o objeto que gerou o gráfico relativo em formato .Rdata, usado posteriormente para montagem de outros tipos de gráficos
}
