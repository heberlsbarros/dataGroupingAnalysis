graficos_significancia_tripla_triplasjuntas <- function (nome_da_planilha, tituloImagem, resposta) {
  ...
  ##Criando um sistema de loops e condicionais aninhados para atribuir o nome correto à coluna de combinações
  
  for (i in 1:nrow(nome_da_planilha)) { #Criando um sistema de ciclo para passar por todas as linhas do dataframe "nome_da_planilha"
    if (nome_da_planilha$droga1[i] == "CT DMSO"){ #Criando uma condicional para checar se o elemento "i" da coluna "droga1" não é CT DMSO
      nome_da_planilha$nome_comb <- "CT DMSO"
    } else if (nome_da_planilha$droga1[i] != "CT DMSO" & nome_da_planilha$droga1[i] != "Nada") {
        nome_da_planilha$nome_comb[i] <- paste(nome_da_planilha$droga1[i], " (", nome_da_planilha$concentracao_droga1[i], ") + ", nome_da_planilha$droga2[i], " (", nome_da_planilha$concentracao_droga2[i], ") + ", nome_da_planilha$droga3[i], " (", nome_da_planilha$concentracao_droga3[i], ")", sep = "")
    }
  }   
...
nome_da_planilha <- nome_da_planilha %>% dplyr::filter (!grepl("TRUE", outlier))
  ...
  nome_da_planilha$nome_comb <- factor(nome_da_planilha$nome_comb, levels = c ("CT DMSO", nome_comb[1], nome_comb[2], nome_comb[3], nome_comb[4], nome_comb[5], nome_comb[6], nome_comb[7]), ordered = T)
 ...
 ############
 porcentagens_individual$nome_comb <- factor(porcentagens_individual$nome_comb, levels = c ("CT DMSO", nome_comb[1], nome_comb[2], nome_comb[3], nome_comb[4], nome_comb[5], nome_comb[6], nome_comb[7]), ordered = T)
 ...
 grafico_porcentagem <- grafico_porcentagem + 
   scale_y_continuous ( #Configura o tamanho do eixo Y.
     limits = c(0, 500), # Define os limites reais de todos elementos que serão mostrados no eixo Y.
     breaks = 0:6 * 50, # Define quais pontos do eixo Y serão mostrados
     expand = expansion(mult = c(0, 0)) # Ativar essa linha cria um pequeno espaço entre a linha de 0 ("0") do eixo y e a linha do eixo x. Se remover, o eixo x coincide com o ponto 0 do eixo y.
   )
 
 grafico_porcentagem <- grafico_porcentagem + 
   annotate("segment", x = 0.5, xend = 0.5, y = 0, yend = 300 #O "yend" define até onde irá a linha do eixo y, mas não define o seu tamanho.
   ) + #Essa #Essa parte de modificação de tema do eixo Y *deve* ficar no final, pois ela fará a modificação do tamanho do eixo y, retirando o valor acima do definido como limite superior no "yend" do "annotate". Se ela for movida para um trecho anterior do código, ela será sobreposta por outras camadas e perderá sua função.
   theme(axis.line.y = element_blank(),
         axis.title.x = element_blank(),# Ajusta a posição do título do eixo x
         axis.text.x = element_text(angle = 45, hjust = 1.0),
...
##Exportando "lista_dados_estatisticos" como um arquivo excel (.xlsx)
 ...
 saveWorkbook(planilha_existente, file = paste(getwd(),"/relatorios_tod_triplas/tod_triplas_compilado_estatistico_", tituloImagem, ".xlsx", sep=""), overwrite = T) #Salva o arquivo planilha
 
 #Salvamento automático dos gráficos
 # ggsave(paste(getwd(),"/graficos_tod_triplas/boxplot_tod_triplas_",tituloImagem,".jpg", sep=""), plot = grafico_boxplot, width = 10, height = 7, dpi = 500) #Exporta a imagem de gráfico de boxplot
 # ggsave(paste(getwd(),"/graficos_tod_triplas/qqplot_tod",tituloImagem,".jpg", sep=""), plot = qqplot_facet, width = 10, height = 7, dpi = 500) #Exporta a imagem de gráfico de qqplot
 # ggsave(paste(getwd(),"/graficos_tod_triplas/triplas_colunasAbsoluto_",tituloImagem,".jpg", sep=""), plot = grafico_grupos, width = 10, height = 7, dpi = 500)
 ggsave(paste(getwd(),"/graficos_tod_triplas/triplas_colunasRelativo_",tituloImagem,".jpg",sep=""), plot = grafico_porcentagem, width = 10, height = 7, dpi = 500)
 # write.csv(nome_da_planilha, paste("./dataframes_tod_triplas/triplas_",tituloImagem, ".csv", sep = ""), row.names = F, quote = F)
 save(grafico_grupos, file = paste(getwd(),"/objetos_graficos/tod_triplas_fluorescencia_",tituloImagem, ".RData", sep = ""))
 save(grafico_porcentagem, file = paste(getwd(),"/objetos_graficos/tod_triplas_porcentagens_",tituloImagem, ".RData", sep = ""))
}
