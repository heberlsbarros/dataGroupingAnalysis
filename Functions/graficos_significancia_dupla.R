graficos_significancia_dupla <- function (nome_da_planilha, tituloImagem, resposta, grupo) {
  ...
  for (z in 1:length(resumo_grupos$droga2)){ #Loop para calcular todos valores de conversão para porcentagem, levando a média de resposta do CT DMSO como referência, tanto para conversão de média quanto para desvio padrão.
  ...
  porcentagens_media <- as.numeric(c(coleta_media[1], coleta_media[2], coleta_media[3], coleta_media[4])) #Tirando o resultado do loop do formato de lista e colocando no formato númerico
  ...
  porcentagens_grupos <- cbind(resumo_grupos$droga2, porcentagens_media) #Agregando o resultado de porcentagens à identificação dos grupos
  ...
  for (z in 1:length(nome_da_planilha$droga2)){ #Loop para calcular todos valores de conversão para porcentagem, levando a média de resposta do CT DMSO como referência, tanto para conversão de média quanto para desvio padrão.
    conversao_media_individual <- (nome_da_planilha$luminescencia[z] *100)/resumo_grupos$media[1] # Aqui é uma fórmula que assume os diferentes valores de resposta, multiplica por 100 e divide pelo valor da média do grupo CT DMSO.
  ...
  porcentagens_individual <- cbind(nome_da_planilha$droga2, porcentagens_individual) #Agregando o resultado de porcentagens à identificação dos grupos
  colnames(porcentagens_individual) <- c("droga2", "porcentagem")
  ...
  #############=========CONFECÇÃO DO QQPLOT=============################
  qqplot_facet <- ggplot(nome_da_planilha, aes(sample = luminescencia)) +
    stat_qq() +
    stat_qq_line(color = "blue") +
    facet_grid(. ~ droga2) + # Separar os QQ plots por tratamento
    labs(
 ...
 id_porcent <- as.data.frame(unique(resumo_porcentagens$droga2)) #criando objeto com os nomes dos grupos, que constarão na identificação das linhas (por algum motivo não aceitou o que já estava pronto no objeto "resumo_grupos")
 ...
 grafico_porcentagem <- ggplot(porcentagens_individual, aes(x = droga2, y = porcentagem, fill = droga2)) + #Ao adicionar o fill com a referência à coluna "droga2" do dataframe "nome_da_planilha", as barras foram coloridas automaticamente. Verificar as cores presentes no pacote "RColorBrewer" com o comando "colors", escolher 4 cores, criar uma nova coluna e associar cada cor a um grupo do "droga2"
   stat_summary( #Ao anexar a barra de erro na segunda camada, a camada seguinte fará a sobreposição na parte inferior da barra de erro.
     fun.data = mean_sdl, #Linha de desvio padrão para cada barra
     fun.args = list(mult = 1), #? QUAL É O OBJETIVO DESSA LINHA?
     geom = "errorbar",
   ...
   labs(x = "Tratamento", y = "Porcentagem referente ao \n CT DMSO")
 ...
 grafico_porcentagem <- grafico_porcentagem + 
   scale_y_continuous ( #Configura o tamanho do eixo Y.
     limits = c(0, expansao_eixoy_anotacao_porcent), # Define os limites reais de todos elementos que serão mostrados no eixo Y.
     breaks = 0:4 * tique_porcent, # Define quais pontos do eixo Y serão mostrados
     expand = expansion(mult = c(0, 0)) # Ativar essa linha cria um pequeno espaço entre a linha de 0 ("0") do eixo y e a linha do eixo x. Se remover, o eixo x coincide com o ponto 0 do eixo y.
   )
 
 grafico_porcentagem <- grafico_porcentagem + 
   annotate("segment", x = 0.5, xend = 0.5, y = 0, yend = barra_sig_porcent #O "yend" define até onde irá a linha do eixo y, mas não define o seu tamanho.
   ) + #Essa #Essa parte de modificação de tema do eixo Y *deve* ficar no final, pois ela fará a modificação do tamanho do eixo y, retirando o valor acima do definido como limite superior no "yend" do "annotate". Se ela for movida para um trecho anterior do código, ela será sobreposta por outras camadas e perderá sua função.
   theme(axis.line.y = element_blank()
   )
 ...
 id_arquivo <- as.data.frame("fazer na mão")
 colnames (id_arquivo) <- c("Planilha original")
 ...
 vetor_grupo <- as.character(resumo_grupos$droga2) #Extrair o conteúdo dos grupos como caractere para um novo objeto
 ...
 vetor_resposta <- c(resumo_grupos$media, NA, NA)
 
 vetor_porcentagem <- c(resumo_porcentagens$media, NA, NA)
 
 lista_dados_estatisticos <- cbind.data.frame(id_arquivo, tituloImagem, metodo_shapiro, levene_result$`Pr(>F)`[1], teste_e_posteste,pos_teste_df$Pairs, pos_teste_df$`p adj`, pos_teste_df$significancia, vetor_grupo, vetor_resposta, vetor_porcentagem)
 colnames(lista_dados_estatisticos) <- c("Planilha Original", "Grupos analisados", "Menor valor de p de shapiro", "Teste de Levene", "Teste e pós-teste estatísticos usados", "Comparações", "Valor de p das comparações", "Significância", "Grupos Individuais", "Media de resposta dos grupos", "Porcentagem de resposta em relação ao CT DMSO")
 ...
 saveWorkbook(planilha_existente, file = paste(getwd(),"/relatorios/", "compilado_estatistico_", tituloImagem, ".xlsx", sep=""), overwrite = T) #Salva o arquivo planilha
 
 #Salvamento automático dos gráficos
 #ggsave(paste(getwd(),"/graficos/boxplot_",tituloImagem,".jpg", sep=""), plot = grafico_boxplot, width = 10, height = 7, dpi = 500)
 ggsave(paste(getwd(),"/graficos/colunasAbsoluto_",tituloImagem,".jpg", sep=""), plot = grafico_grupos, width = 10, height = 7, dpi = 500)
 ggsave(paste(getwd(),"/graficos/colunasRelativo_",tituloImagem,".jpg",sep=""), plot = grafico_porcentagem, width = 10, height = 7, dpi = 500)
}
